...
func (r *ocmComponentsResolver) writeLandscapeKitComponents() error {
	type componentsOutput struct {
		// Components is a map of component names to their versions.
		Components map[string]string `json:"components"`
	}

	componentVersions, err := r.components.GetGLKComponentVersionMap(r.cfg, false)
	if err != nil {
		return err
	}
	data, err := yaml.Marshal(&componentsOutput{
		Components: componentVersions,
	})
	if err != nil {
		return fmt.Errorf("failed to marshal component versions to YAML: %w", err)
	}
	filename := path.Join(r.landscapeDir, "ocm-component-versions.yaml")
	if err := os.WriteFile(filename, []byte(data), 0600); err != nil {
		return fmt.Errorf("failed to write component version file %s: %w", filename, err)
	}
	r.log.Info(fmt.Sprintf("Wrote component version file to %s", filename))
	return nil
}
