...
registryIP="$(docker inspect $registryHostname | yq ".[].NetworkSettings.Networks.kind.IPAddress")"
# Inject registry.local.gardener.cloud into all nodes
for node in $nodes; do
  docker exec "$node" sh -c "echo $registryIP registry.local.gardener.cloud >> /etc/hosts"
done

# Inject registry.local.gardener.cloud into coredns config (after ready plugin, before kubernetes plugin)
kubectl -n kube-system get configmap coredns -ojson | \
  yq '.data.Corefile' | \
  sed '0,/ready.*$/s//&'"\n\
    hosts {\n\
      $registryIP registry.local.gardener.cloud\n\
      fallthrough\n\
    }\
"'/' | \
...
for node in $nodes; do
  setup_containerd_registry_mirror $node "registry.local.gardener.cloud:5001" "http://registry.local.gardener.cloud:5001" "http://${registryHostname}:5001"
