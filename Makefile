...
.PHONY: git-server-up
git-server-up:
	@bash $(HACK_DIR)/git-server/git-server-up.sh

.PHONY: git-server-down
git-server-down:
	@bash $(HACK_DIR)/git-server/git-server-down.sh

.PHONY: git-server-cleanup # cleanup git server data
git-server-cleanup: git-server-down $(YQ)
	@rm -rf $(HACK_DIR)/git-server/data

.PHONY: registry-up
registry-up:
	@bash $(HACK_DIR)/registry/registry-up.sh

.PHONY: registry-down
registry-down:
	@bash $(HACK_DIR)/registry/registry-down.sh
